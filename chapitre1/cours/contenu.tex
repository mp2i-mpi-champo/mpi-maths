...
\begin{definition}{Valeur absolue}{}
    $\forall x \in \R, \mid x \mid = \max\{x, -x\}$
\end{definition}
...
\begin{exemple}
    ...
    \svspace Soient $x \in A$ et $y \in B$ 
    ...
    \svspace Soit $(u_n)_{n \in N}$ suite de $A$ telle que $u_n \longrightarrow \sup A$
...
\begin{definition}{Définition d'une norme}{}
    On appelle norme sur $E$ toute application 
    \danger{Bien penser à vérifier que c'est une application à valeurs dans $\R^+$ pour montrer qu'on a une norme !}
    ...
    \begin{itemize}
        ...
        \item Homogénéité : $\forall u \in E, \forall \lambda \in \K, N(\lambda u) = \mid \lambda \mid N(u)$
...
\begin{proof}
    ...
    $d_N(u, w) = N(u - w) = N( u - v + v - w) \leqslant N(u - v) + N(v - w)$ \\
...
\begin{definition}{Définition vecteur unitaire}{}
...
\begin{exemple}
    ...
    Si $\K = \C$, tous les $e^{i \theta}u$, où $\theta \in \R$, conviennent. 
\end{exemple}
...
\begin{exemple}
    Dans $(\C, \mid \cdot \mid)$ les boules fermées sont les disques, et les sphères sont les cercles. 

    Dans $(\R, \mid \cdot \mid)$, les boules fermées sont les segments non 
...
\begin{definition}{Barycentre - HP}{}
    ...
    Ainsi $\inter{a}{b}$ est l'ensemble des barycentres de $a$ et $b$ à coefs positifs. 
\end{definition}

\begin{proposition}{Nature d'une intersection de convexes - légèrement HP}{}
...
\begin{definition}{Enveloppe convexe - hors programme}{}
    Soit $A \subset E$ non vide. 

    L'intersection $B$ de tous les convexes de $E$ contenant $A$ est donc un convexe
    de $E$ contenant $A$ appelée enveloppe convexe de $A$.
\end{definition}

\begin{theoreme}{Enveloppe convexe avec les barycentres - hors programme}{}
    Avec ces notations, l'enveloppe convexe de $A$ est l'ensemble des barycentres à
    coefs positifs de points de $A$. 
\end{theoreme}
...
\begin{exemple}
    ...
    Donc $\displaystyle \sum_{i = 1}^{k }\frac{m_i }{\mid \alpha - z_i \mid ^2} (\alpha - z_i) = 0$
    
    En posant $\beta = \frac{m_i}{\mid \alpha - z_i \mid ^2}$
...
\idee{
    Astuce du topin, utiliser la distance au centre
    en faisant apparaitre le rayon.
}

\begin{proof}
    \quad \underline{Boule fermée :}
    ...
    \begin{align*}
        ...
        & \leqslant \, \mid \lambda \mid N(a - \omega) + \mid 1 - \lambda \mid N(b - \omega) \\
        & \leqslant \lambda R + (1 - \lambda)R = R \\
    \end{align*}

    \quad \underline{boules ouvertes : }
    ...
    \begin{align*}
        ...
        & = N(\lambda(a - \omega) + (1 - \lambda) (b - \omega) \\
        & \leqslant \, \mid \lambda \mid N(a - \omega) + (1 - \lambda)N(b - \omega)) \\
        & < (\mid \lambda \mid + \mid 1 - \lambda \mid) R = R
    \end{align*}
\end{proof}
...
\idee{
    Astuce du topin, on passe par $\omega$ et inégalité triangulaire pour majorer. 
    
    Pour minorer, carac séquentielle, distance entre éléments de deux suites.
}
    ...
    \begin{itemize}
        ...
        \item Soient $f, g \in \mathscr B(X, E)$, $\alpha, \beta \in \K$. Par hypothèse, 
            ...
            \begin{array}{rclr}
                ...
                \space & \leqslant & \mid \alpha \mid N(f(x)) + \mid \beta \mid N(g(x)) \\
                \space & \leqslant & \mid \alpha \mid M_1 + \mid \beta \mid M_2\\
            \end{array}
            \]
    \end{itemize}
...
\section{Equivalence des normes}
...
\begin{remarque}
    ...
    Si on a seulement $\exists \alpha > 0, \forall x \in E, N_1(x) \leqslant \alpha N_2(x)$
...
\begin{proposition}{Equivalence bornage norme}{}
...
\begin{proposition}{Montrer que deux normes ne sont pas équivalentes}{}
    ...
    $$ \frac{N_1(u_n )}{N_2(u_n )} \limit{n}{+ \infty} 0$$ 
    ou $$\frac{N_2(u_n )}{N_1(u_n )} \limit{n}{+ \infty} 0$$
...
\subsubsection{Dans \texorpdfstring{$\mathscr C([0, 1], \R)$}{C([0, 1], R)}}
...
\begin{proof}
    ...
    \quad \underline{Si $k = 0$ :}

    C'est évident, $kA = \{0\}$

    \quad \underline{Sinon : }
...
\begin{definition}{Norme infinie sur Kn}{}
    Pour $X = \{1, \dots, n\}$ et en utilisant via la bijection $\mathscr B(X, \K)$
    à $\K^n$ 
