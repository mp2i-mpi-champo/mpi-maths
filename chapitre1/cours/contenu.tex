...
\begin{proof}
    ...
    $d(x, a) \leqslant d(x, y) + d(y, a)$, or $d(x, a) \geqslant d_A(x)$
...
\begin{remarque}
    Une suite qui marche souvent est $(x^n)_{n \in \N}$.
\end{remarque}
...
\subsubsection{Norme infinie sur $\K^n$}
...
\subsubsection{Sur $\ell^1$ $\K$-ev des suites sommables sur $\K$}

\begin{definitionnt}{}{}
    \\On note $\ell^1(\K) = \{(u_n)_{n \in \N} \in \K^n \mid \sum_{n \leqslant 0} |u_n| ~ \text{est une suite convergente} ~ \}$
\end{definitionnt}
...
\subsubsection{Sur $\mathscr C([a, b], \K)$}
...
\begin{remarque}
    Si on prend le produit scalaire $\langle f, g \rangle = \int_{a}^{b} fg$, on a 
    $f(x) =  \langle f', 1 \rangle $ (peut servir en exercice). 
\end{remarque}
...
\begin{proof}
    ...
    \begin{itemize}
        \item Si $||f||_1 = 0$ alors $\int_{a }^{b } |f(t)| \text d t = 0$
...
\begin{exemple}
    ...
    On peut poser pour $P \in E$ $||P|| = \int_{-1}^{2} |P(t)| \text d t$ \\
...
\begin{proof}
    Par la démo précédente, $\langle u, v \rangle$ existe, est bilinéaire symétrique par calcul 
...
\begin{proof}
    ...
    Donc $\mid \langle u, v \rangle \leqslant ||u||_2 \cdot ||v||_2$ qui est l'inégalité de Cauchy Schwarz. 
...
\begin{exemple}
    ...
    Et donc $\ell^2(\K)$ sev de $\K^{\N}$ car $|u_n + v_n|^2 \leqslant |u_n|^2 + 2 |u_n v_n| + |v_n|^2$ \\
